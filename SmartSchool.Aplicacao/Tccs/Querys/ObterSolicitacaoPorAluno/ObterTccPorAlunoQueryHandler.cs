...
{
    public class ObterTccPorAlunoQueryHandler : IRequestHandler<ObterTccPorAlunoQuery, IResult>
    {
        private readonly IRepositorio<TccAlunoProfessor> _tccRepositorio;

        public ObterTccPorAlunoQueryHandler(IRepositorio<TccAlunoProfessor> tccRepositorio)
        {
            _tccRepositorio = tccRepositorio;
        }

        public async Task<IResult> Handle(ObterTccPorAlunoQuery request, CancellationToken cancellationToken)
        {
